% Compilar con: pdflatex Informe_Divulgativo_Laboratorio_2.tex
...
Las demás categorías se dejaron fuera para conservar exactamente la misma
definición utilizada en el Laboratorio 1. De esta manera, la comparación entre
los modelos tradicionales y las LSTM se realiza sobre las mismas personas y
los mismos meses.
...
Además, se documentaron dos diseños de mayor tamaño que se probaron en ambas
series y luego se descartaron por su menor estabilidad. Por ello, la elección
final no se basó solamente en la cifra más baja de una prueba aislada.
...
% PÁGINA 7
% ---------------------------------------------------------------------------
\section{catch22: una forma de ``huella digital'' para cada serie}

Más allá de pronosticar, también interesaba entender qué series se comportan
de manera parecida entre sí. Para eso se recurrió a una técnica llamada
\textit{catch22}.

La idea es sencilla de imaginar: cada serie recibe una \textbf{huella digital
de 22 números}, donde cada número resume un aspecto distinto de su
comportamiento en el tiempo, por ejemplo:

\begin{itemize}
  \item qué tanto se repiten o dependen los valores entre meses cercanos;
  \item si existen ciclos o patrones estacionales;
  \item si hay meses inusualmente altos o bajos;
  \item qué tan rápido o lento cambia la serie;
  \item qué tan estables o volátiles son sus fluctuaciones.
\end{itemize}

Se calculó esta huella para siete series distintas:

\begin{itemize}
  \item el total nacional;
  \item las regiones América Del Centro, América Del Norte y Europa;
  \item las fronteras La Aurora, Valle Nuevo y San Cristóbal.
\end{itemize}
...
En este mapa de calor, los colores rojo y azul no significan ``bueno'' o
``malo''; simplemente muestran si una característica está por encima o por
debajo del promedio de las siete series analizadas.

\callout{Teal}{%
\textbf{¿Para qué sirve esto?} Dos series pueden tener cantidades muy
distintas de visitantes y, aun así, moverse con una forma parecida en el
tiempo. catch22 permite comparar esa forma, independientemente de la escala
de cada serie.
}
...
% PÁGINA 8
% ---------------------------------------------------------------------------
\section{¿Qué series terminaron pareciéndose más?}

Las 22 características de cada serie se resumieron en un mapa de dos
dimensiones. En ese mapa, los puntos que quedan más cerca representan series
con comportamientos más similares.

\begin{center}
\includegraphics[width=0.73\textwidth]{catch22_pca.png}
\end{center}

\subsection{Los hallazgos más relevantes}

\begin{itemize}
  \item \textbf{El par más parecido} fue el total nacional con América Del
  Centro.
  \item \textbf{La serie más distinta} de todas resultó ser 01 La Aurora.
  \item \textbf{Grupo 1}: total nacional, América Del Centro, Valle Nuevo y
  San Cristóbal.
  \item \textbf{Grupo 2}: América Del Norte y Europa.
  \item \textbf{Grupo 3}: La Aurora, sola, sin acompañamiento cercano.
\end{itemize}

Que el total nacional quede cerca de América Del Centro es coherente con el
peso que tienen los mercados centroamericanos y las fronteras terrestres. La
Aurora, en cambio, se aparta del resto porque depende directamente de los
vuelos internacionales y vivió una caída y una recuperación con un ritmo muy
particular.

\subsection{Un hallazgo que vale la pena resaltar}

Pertenecer a la misma categoría administrativa no garantiza un comportamiento
parecido. La Aurora, Valle Nuevo y San Cristóbal son todas fronteras, pero La
Aurora no terminó agrupada con las otras dos. Esto sugiere que la forma en
que cambia una serie a lo largo del tiempo puede decir más sobre ella que la
simple etiqueta que se le asigne.

\callout{Gold}{%
Este análisis se hizo con solo siete series, así que los grupos encontrados
deben tomarse como una guía exploratoria y no como una clasificación
definitiva de todas las fronteras o mercados del país.
}
...
% PÁGINA 9
% ---------------------------------------------------------------------------
\section{¿Ayudó agregar catch22 a mejorar el pronóstico?}

Se construyó un modelo adicional, esta vez para La Aurora, que además de
observar los últimos 12 meses también recibía la huella catch22 calculada con
los 36 meses previos.
...
\subsection{El resultado no fue el esperado, pero sí fue informativo}

Agregar catch22 \textbf{no mejoró} el pronóstico: su RMSE (42,907) quedó por
encima del de la LSTM base (31,020). En otras palabras, la versión más
compleja no resultó más precisa.

Algunas razones probables:

\begin{itemize}
  \item había pocos ejemplos disponibles para que el modelo aprendiera a usar
  23 entradas simultáneas en lugar de solo una;
  \item varias de las 22 características resultaron redundantes entre sí;
  \item una huella pensada para \emph{comparar} series completas no
  necesariamente aporta valor para \emph{pronosticar} mes a mes.
\end{itemize}

\callout{Teal}{%
\textbf{Por qué este resultado importa igual.} Confirmar que algo no funciona
también es un aprendizaje válido: evita invertir esfuerzo en una solución más
complicada cuando la alternativa sencilla ya rinde mejor.
}
...
\subsection{Lo que se puede afirmar con los datos disponibles}

\begin{enumerate}
  \item Se trabajó con 210 meses completos de dos series de visitantes.
  \item Se compararon cuatro variantes finales de LSTM por cada serie y se
  documentaron dos diseños adicionales que fueron descartados.
  \item La LSTM redujo el error en La Aurora en 26.24\% frente a SES.
  \item En el total nacional, la mejora frente a Prophet fue de 1.58\%.
  \item El total nacional y América Del Centro resultaron ser las series más
  parecidas entre sí.
  \item La Aurora fue, de las siete series analizadas, la de comportamiento
  más particular.
  \item Sumar catch22 a la LSTM no mejoró la calidad del pronóstico.
\end{enumerate}

\subsection{Recomendaciones para usar esto en la práctica}

\begin{itemize}
  \item Reentrenar los modelos cada vez que se incorporen meses nuevos a la
  base.
  \item Conservar más de un método disponible; no depender de un único
  algoritmo.
  \item Separar los pronósticos por frontera o región cuando se necesite
  planificar recursos a nivel local.
  \item Explorar la incorporación de vuelos, feriados, restricciones y
  variables económicas como información adicional.
  \item Comunicar los resultados como rangos o escenarios, no como una única
  cifra exacta.
  \item Dejar constancia de cualquier cambio futuro en la forma de registrar
  visitantes, para no romper la comparabilidad histórica.
\end{itemize}

\subsection{Glosario mínimo}

\begin{tabularx}{\textwidth}{>{\bfseries\color{Blue}}p{2.8cm}X}
LSTM & Técnica que aprende patrones en datos ordenados en el tiempo y
conserva memoria de meses anteriores.\\
Pronóstico & Estimación de un valor que todavía no se conoce.\\
RMSE & Medida de error que penaliza con más fuerza las equivocaciones
grandes.\\
catch22 & Conjunto de 22 números que resume la forma temporal de una
serie.\\
PCA & Mapa simplificado que permite observar semejanzas entre muchas
características a la vez.\\
Clustering & Procedimiento que agrupa elementos con comportamientos
parecidos.\\
\end{tabularx}
